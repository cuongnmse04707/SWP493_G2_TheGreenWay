...
let addNewOrderByGuest = async (req, res) => {
    // Render guestID
    const day = new Date(); 
    const guestID = (day.getFullYear()+"-"+day.getMonth()+"-"+day.getDate()+"-"+day.getMilliseconds()+"-"+day.getMinutes()+"-"+day.getHours());
    ...
    connectionDB.query('INSERT INTO Guest SET ? ',emptyGuess, (err, result) => {
        if (err){
            debug(err);
            return res.status(200).json({success: false,message : "Add Guest is Unsuccess!"});
        }else{
                ...
                if (err) {
                    ...
                    connectionDB.query('INSERT INTO Orders SET ? ',empty, (err, result) => {
                        if (err) {
                          ...
                          // Luu vao Database
                          connectionDB.query('INSERT INTO OrderStatusDetail SET ? ',emptyOrderStatusDetail, async (err, result) => {
                              if (err) {
                                  debug(err);
                                return res.status(200).json({success: false,message : "Add New Orders is Unsuccess!"});
                              }else{
                                ...
                                try {
                                    ...
                                    const accessToken = await jwtGuest.generateTokenGuest(guestData, accessTokenSecret, accessTokenLife);
                                    ...
                                    // Luu vao Database
                                    connectionDB.query('INSERT INTO GuestToken SET ? ',emptyGuest, (err, result) => {
                                        if (err){
                                            debug(err);
                                            return res.status(200).json({success: false,message : "Add Guest is Unsuccess!"});
                                        }else{
