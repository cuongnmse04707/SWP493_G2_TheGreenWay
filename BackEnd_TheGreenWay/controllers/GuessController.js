...
let addNewOrderByGuest = async(req, res) => {
    ...
    connectionDB.query(query, async(err, result) => {
        if (err) {
            ...
            var checkOrder = true;
            arr.forEach((e, index) => {
                const arrItem = arrayCart.find(el => el.id === e.ProductID)
                if (e.Quantity >= arrItem.quatityBuy) {} else {
                    // e.Quantity >= arrayCart[index].quatityBuy
                    checkOrder = false;
                    if (e.Quantity === 0) {
                                                        ...
                                                        if (err) {
                                                            ...
                                                            try {
                                                                ...
                                                                const mailOptions = {
                                                                    ...
                                                                    text: "Bạn đang nhận được điều này bởi vì bạn (hoặc người khác) đã yêu cầu đặt hàng bằng email của bạn.\n\n" +
