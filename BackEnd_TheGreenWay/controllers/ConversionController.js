...
let getInfoConversionbyID = async (req, res) => {
    ...
    connectionDB.query(query , async (err, result) => {
        if (err) {
            ...
            if(arr.length===0){
                return res.status(200).json({success: false,message : "No conversion tables exist !"});
            }else{
...
let addNewConversion = async (req, res) => {
    ...
    connectionDB.query(query , async (err, result) => {
        if (err) {
            ...
            connectionDB.query('INSERT INTO ConversionPaper SET ? ',empty, (err, result) => {
                if (err) {
                    debug(err);
                    return res.status(200).json({success: false,message : "Add New Conversion is Unsuccess!"});
                }else{
                    if(req.body.Status==="Đang Hoạt Động"){
                        ...
                        connectionDB.query(query , async (err, results) => {
                            if (err) {
                                ...
                                return res.status(200).json({success: true,message : "Add New Conversion is Success!"});
                            };
                        });
                    }else{
                        return res.status(200).json({success: true,message : "Add New Conversion is Success!"});
...
let updateConversion = async (req, res) => {
    ...
    let query = mysql.format(sql, [PaperPrice,CreatedAt,ModifyDate,Status,idConversion]);
    connectionDB.query(query , async (err, result) => {
        if (err) {
            return res.status(200).json({success: false,message : "Update Conversion Failed!"});
        }else{
            if(req.body.Status==="Đang Hoạt Động"){
                ...
                connectionDB.query(queryNew , async (err, results) => {
                    if (err) {
                        return res.status(200).json({success: false,message : "Update Conversion is Unsuccess!"});
                    }else{
                        return res.status(200).json({success: true,message : "Update Conversion is Success!"});
                    };
                });
            }else{
                return res.status(200).json({success: true,message : "Update Conversion is Success!"});
...
let getConversionWorking = async (req, res) => {
    ...
    connectionDB.query(query , async (err, result) => {
        if (err) {
            return res.status(200).json({success: false,message : "Get Conversion Working Failed!"});
        }else{
            const arr = await Array.apply(null,result);
            if(arr.length===0){
                return res.status(200).json({success: false,message : "No Conversion Working exist !"});
            }else{
                return res.status(200).json({
                    success: true,
                    data : arr[0]
...
let getListConversion = async (req, res) => {
    ...
    connectionDB.query(query , async (err, result) => {
        if (err) {
            return res.status(200).json({success: false,message : "Get List Conversion Failed!"});
        }else{
            const arr = await Array.apply(null,result);
            if(arr.length===0){
                return res.status(200).json({success: false,message : "No Conversion exist !"});
            }else{
