...
let getPayment = async (req, res) => {
    ...
    connectionDB.query(query , async (err, result) => {
        if (err) {
            ...
            if(arr.length===0){
                return res.status(200).json({success: false,message : "No Payment exist !"});
            }else{
...
let addNewPayment = async (req, res) => {
    ...
    connectionDB.query(query , async (err, result) => {
        if (err) {
            ...
            connectionDB.query('INSERT INTO PaymentMethods SET ? ',empty, (err, result) => {
                if (err) {
                    return res.status(200).json({success: false,message : "Add New PaymentMethod is Unsuccess!"});
                }else{
                    return res.status(200).json({success: true,message : "Add New PaymentMethod is Success!"});
                };
            });
        };
    });
...
let updatePayment = async (req, res) => {
    ...
    connectionDB.query(query , async (err, result) => {
        if (err) {
            return res.status(200).json({success: false,message : "Update Payment Failed!"});
        }else{
            return res.status(200).json({success: true,message : "Update Payment is Success!"});
        };
    });
}
...
let detelePayment = async (req, res) => {
    ...
    connectionDB.query(query , async (err, result) => {
        if (err) {
            return res.status(200).json({success: false,message : "Remove Payment Failed!"});
        }else{
            return res.status(200).json({success: true,message : "Remove Payment Success!"});
        };
    });   
}
